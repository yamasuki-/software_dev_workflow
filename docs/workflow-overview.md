...
## エージェント構成 (Skill 5 個 + Agent 26 個)

ユーザは Skill `dev-workflow` (または `dev-workflow-overlay`) を起動する。単発の不具合修正・機能追加には軽量な派生 Skill `bugfix-workflow` / `feature-add-workflow` を使う (解析 → 対応方法提案・ユーザ選定 → 設計差分・ユーザ承認 → TDD 実装の 4 ステップ。Agent 群を再利用)。設計書のない既存コードからの設計復元には `reverse-design-workflow` を使う (コード → 詳細設計 → 基本設計 → 要件を逆順で起こし、各層を適合性テストで検証。ソース修正禁止)。Skill が「メイン Claude」として動き、各フェーズの作業を Agent (`~/.claude/agents/<name>/<name>.md`) に `Task(subagent_type="<name>")` で委譲する。

```mermaid
flowchart TD
    User["ユーザ"] <--> Orch["dev-workflow Skill<br/>(オーケストレータ)"]
    Orch -. "Task(subagent_type=...)" .-> RQA["requirements Agent"]
    Orch -. spawn .-> BDA["basic-design Agent"]
    Orch -. spawn .-> DDA["detailed-design Agent"]
    Orch -. spawn .-> TDA["test-design Agent"]
    Orch -. spawn .-> TIA["test-implementation Agent<br/>TDD Red"]
    Orch -. spawn .-> TR["testing Agent<br/>mode=red/green 確認"]
    ...
    Orch -. spawn .-> RevA["*-review Agents (11)<br/>requirements-review / security-review 含む"]
...
| タイミング | 対象 |
|---|---|
| `requirements-review` pass 直後 | 要件定義書 (R-### / 受入条件 / スコープ境界) の確定 |
...
flowchart TD
    ...
    B --> RQ["requirements<br/>要件ID・受入条件の確定"]
    RQ --> RQR{"requirements-review"}
    RQR -->|fail| RQ
    RQR -->|pass| CP0{"🛑 human-checkpoint<br/>requirements 承認"}
    CP0 -->|変更要求| RQ
    CP0 -->|approve / skip| C["basic-design"]
    ...
    subgraph Batch ["フェーズバッチ: 同じフェーズを全機能まとめて → 2 段レビュー"]
      ...
      IR2 --> SR1{"security-review per_feature<br/>OWASP/秘密情報/設定"}
      SR1 -->|fail| ImplBatch
      SR1 -->|"all pass"| SR2{"security-review cross<br/>依存脆弱性/横断一貫性"}
      SR2 -->|fail| ImplBatch
      SR2 --> TestUnit["testing layer=unit<br/>全機能 並行"]
...
## ゲートの動作 (testing red/green → auto-check → per_feature → cross)

各フェーズ完了直後、LLM レビューの前段で **testing mode=red/green (実装系のみ) → auto-check → per_feature → cross** の順に走る。確認モードは実装系フェーズ (test-implementation / implementation) でのみ spawn される。

```mermaid
flowchart TD
    Phase[フェーズ作業 全機能 並行 spawn]
    Phase --> TR{実装系フェーズ?}
    TR -->|test-implementation| TR1[testing mode=red N並行]
    TR -->|implementation| TR2[testing mode=green N並行]
...
- **testing (mode=red/green)**: 対象機能のテストを走らせ Red/Green 状態だけを確認 (旧 test-run Agent を統合)。`docs/04_test_results/<FID>/<phase>-<mode>-confirmation.md` に出力
- これにより `test-implementation-review` / `implementation-review` は **自分でテスト実行しなくてよい** (testing の確認レポートを Read するだけ)
...
| -------- | ------------------------ | ---------------------------------------------------- |
| 単体     | **詳細設計** `detailed-design.md` (+任意の ui/db) | 機能内部の振る舞いが詳細設計どおりに動くこと          |
