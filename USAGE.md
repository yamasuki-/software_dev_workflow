# dev-workflow スキルセット — 使い方 (USAGE)

> 概要・構成・リポジトリ構造・ID 規約などは [README.md](./README.md) を参照。本書はインストールから運用・カスタマイズまでの手順集。
> 仕様の正 (single source of truth) は各 `SKILL.md`・`agents/<name>/<name>.md`・`skills/dev-workflow/resources/reference/` (README の注記と同じ)。

## 目次

- [新規プロジェクトでの使い方](#新規プロジェクトでの使い方) — インストール → プロジェクト準備 → 起動 → 再開
- [サンプルウォークスルー: タスク管理 API を作る](#サンプルウォークスルー-タスク管理-api-を作る)
- [シナリオ別 起動プロンプト例](#シナリオ別-起動プロンプト例) — 既存プロジェクトの改修 / USDM 形式の要件
- [レガシープロジェクトへの適用](#レガシープロジェクトへの適用) — 方針 A (全面逆引き) / B (部分適用) / C (新機能のみ)
- [プロジェクト固有のカスタマイズ (Overlay)](#プロジェクト固有のカスタマイズ-overlay) — 2 層ルール / extra-phases / stack-presets / auto-check 導入
...
**最重要の制約: カスタム subagent_type が使えない (2026-07 時点の Cowork 研究プレビューで確認)**

Cowork の `Agent` ツールが受け付ける subagent_type は環境提供の固定リスト (`general-purpose` / `Explore` / `Plan` 等) のみで、`~/.claude/agents/` 相当のカスタム Agent (`basic-design` 等の本スキルセットの 26 Agent) は **登録されない**。そのため Cowork では `Task(subagent_type="basic-design")` は失敗し、dev-workflow §「サブエージェント呼び出し仕様」に定めた **general-purpose フォールバックが標準経路** になる:

1. 該当 Agent 定義ファイル (`agents/<name>/<name>.md`) を **作業フォルダ内に配置しておく** (Cowork からは `~/.claude/agents/` が読めるとは限らないため、本リポジトリごと作業フォルダに置くか、プロジェクトの `.dev-workflow/templates/` への集約コピーに定義ファイルも含める)
2. オーケストレータが定義ファイルを Read し、**frontmatter を除いた本文をブリーフ冒頭に貼り付けて** `Agent(subagent_type="general-purpose", ...)` で spawn する
3. フォールバックでは frontmatter の `tools` / `model` 制約が **強制されない** ため、ツール制約 (例: bug-investigation の「恒久的な変更禁止」、review 系の「Read/Write のみ」) を **ブリーフに禁止事項として明記** して補う

この経路は毎回 Agent 定義全文をブリーフに載せるためトークン消費が増え、ツール制約もプロンプト任せになる。**フル機能での利用は Claude Code (CLI / VS Code 拡張) を推奨**。Cowork の仕様は変わり得るため、利用前に「Agent ツールで使える subagent_type を列挙して」と確認するとよい。
...
> この CWD (Cowork では選択フォルダ) がそのまま **プロジェクトルートとして自動で使われる** ため、起動プロンプトで「プロジェクトルート: <パス>」を指定する必要はない。CWD と異なる場所を対象にしたい場合のみ明示する。
...
```
Claude (dev-workflow):
  プロジェクトルートは現在の作業ディレクトリ (task-api) を使います。
...
Claude (オーケストレータ):
  ...
  - 戻り値:
    summary: F001 の詳細設計 (9章構成) を作成 (UI なしのため ui-design.md は作成せず、§9 状態は「該当なし」明記)。
    updated_files:
      - docs/02_detailed_design/F001/detailed-design.md (§1-9: サブ機能・I/F・シーケンス。§9 は該当なし)
...
戻り値:
  ...
  updated_files:
    - .dev-workflow/features/F001/bugs/B001.json
    - docs/05_bug_reports/B001.md
    - docs/02_detailed_design/F001/detailed-design.md    # §5 に title の必須/最小長を明記
...
オーケストレータの応答 (概要):
- F002 の `phases.detailed_design.status` を `in_progress` に戻し、`detailed-design.md` (§5 サブ機能・§7-8 シーケンス) / `db-design.md` の差分修正を `detailed-design` サブエージェントに依頼
...
仕様 (`S-###-##`) は詳細設計の `detailed-design.md` §2 トレーサビリティ・§3 サブ機能一覧の行と、テスト設計の各テストIDの「カバーする要件ID」欄から参照される。
...
5. **逆引き detailed-design** を機能ごとにバッチ spawn (横断レビューも通常どおり)。既存コードを Read して 9章構成の `detailed-design.md` (+必要なら ui-design / db-design) を起こす
...
4. 既存モジュールを呼び出す箇所は、詳細設計 (detailed-design.md §8 シーケンス図) で
...
`stack/` 層 (言語/フレームワーク共通ルール) は、よく使うスタックについて **完成済みのプリセット** を `dev-workflow-overlay` 同梱で 8 種類用意してある。新規プロジェクト立ち上げ時は、該当プリセットを `stack/` にコピーするだけで、その技術スタック向けの規約 9 ファイルが一気に揃う。
...
各プリセットは以下の 9 ファイル構成で統一されている (そのまま `.dev-workflow/rules/stack/` にコピーして使う):
...
| cross | `docs/06_reviews/_cross/<phase>-auto-check.md` |
