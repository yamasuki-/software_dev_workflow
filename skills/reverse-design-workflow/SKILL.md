...
なお、ベース SKILL.md の上記各節は要点のみで、**詳細手順はベースの `resources/reference/` に分割されている**。該当場面ではベース側の対応ファイルを Read してから実行すること (Git 統合 → `git-integration.md`、human-checkpoint → `human-checkpoint.md`、auto-check → `auto-check-gate.md`)。
...
**機能別状態ファイルの作成 (オーケストレータの責務)**: 承認された各 `F###` について、`.dev-workflow/templates/basic-design/status.json` (テンプレ) をコピーして `.dev-workflow/features/<FID>/status.json` を作成する。forward フローでは basic-design Agent が作成するが、**本ワークフローは basic-design Agent を使わないため、作成はこのタイミングでオーケストレータが行う**。適合性検証の結果は各 status.json の `conformance.<layer>` (schema 1.6、`null` = 未実行) に conformance-test が記録する。
