...
- アーキテクチャ・機能分割に影響する場合のみ `basic-design` の該当ドキュメントも差分更新 (その場合は下流の detailed-design も影響 FID で連鎖更新する)
- **設計差分が生じた場合は影響層のテスト設計も整合更新**: `test-design` (差分・影響層のみ) を続けて spawn し、`docs/03_test_design/<FID>/` を修正後の設計に合わせる (bug-fix Step 3 の前工程補強は検出層より細かい層に限られるため、検出層以上のテスト設計整合はここで担保する)
...
3. `bug-fix-review` を spawn:
   ...
   - `fail` → 該当ステップを再実施
4. **影響範囲のテスト (bug.json の `impacted_FIDs` が非空なら必須)**: 仕上げに `testing` (mode=retry, 対象 = 影響機能群, layer = 検出層および影響が及ぶ上位層) でリグレッション全体を確認する。影響範囲は Root Cause の参照元コード・`feature-list.md` の `depends_on`・更新した設計ファイルの属する FID から導出する
...
- 解析の結果、設計レベルの根本欠陥 (機能分割・アーキの誤り) や **要件の誤り (requirements への差し戻しが必要な `requirements_misinterpretation`)** が疑われる場合は、本ワークフローで完結させず `dev-workflow` への移行をユーザに提案する (要件 → 基本設計 → 詳細設計 → テスト設計の連鎖更新はフル dev-workflow の差し戻し機構で行う)
