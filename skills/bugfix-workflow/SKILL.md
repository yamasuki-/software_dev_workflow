...
description: 報告された不具合 1 件を「原因解析 (推測禁止) → 対応方法提案 (ユーザ選定) → 設計修正 (差分明示) → ユーザ承認 → TDD で実装・テスト」の 4 ステップで修正する軽量ワークフローのオーケストレータ。インプットは不具合内容・再現手順・再現率の 3 点。dev-workflow の Agent 群 (bug-investigation / solution-proposal / detailed-design / bug-fix / testing / 各 review) を再利用する。フルの dev-workflow を回すまでもない単発の不具合修正に使用する。
---
...
- **Git 統合**: 開始時の専用ブランチ確認 (main/master では開始しない)、ゲート通過時 commit、push は人、履歴改変禁止
...
1. 2〜4 案 (最小修正案を必ず含む) + 推奨案が返る
2. **オーケストレータがユーザに選択肢を提示** (Cowork では `AskUserQuestion`、推奨案を先頭に)
3. ユーザの選定結果と理由を `decisions.md` に記録し、proposal レポートの「選定結果」を埋める
...
- **既存設計が無い場合**: まず関連箇所のみ **現行コードから逆引きで最小の設計を作成** し (フル 5 種は不要、関連する種類のみ)、その上で変更点を同じ差分表で示す。「逆引き部分 (現行の事実)」と「変更部分」を明確に区別すること
...
1. **🛑 設計差分をユーザに提示して承認を待つ**: 変更ドキュメントのパス一覧、差分表の要約、選定案、影響範囲。応答パターンはベース §「人間チェックポイント」と同じ (approve / 変更要求 / skip checkpoint)。approve 時は `decisions.md` 記録 + **commit** (`[bugfix-workflow] checkpoint: B<NNN> design approved`)
2. **TDD 実装**: `Task(subagent_type="bug-fix", ...)` を **Step 3 から** spawn (調査は完了済み・設計は承認済みであることをブリーフに明記):
   - Step 3: 前工程テスト設計の修正 + テストコード追加 → **修正前コードで Fail (Red) を必ず確認**
   - Step 4: 承認済み設計に沿った最小のコード修正
   - Step 5: 検出元 + 追加分 + 同一機能リグレッション + 横断影響分を実行
3. `bug-fix-review` を spawn:
   - `pass_and_verified` → `status = verified`。**commit** (`[bugfix-workflow] bug-fix B<NNN>: verified`)。完了報告
