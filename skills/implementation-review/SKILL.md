...
### A. テスト Green の確認 (test-run の結果を Read のみ)

> **本レビューはテストを実行しない**。直前に走った **`test-run` (phase=implementation, mode=green)** が出力した結果を Read して判定する。
> test-run が verdict=FAIL を出していた場合、本レビューは spawn されない (オーケストレータが implementation に差し戻す)。

- [ ] `docs/04_test_results/<FID>/implementation-green-confirmation.md` (test-run のレポート) が存在する
...
3. **test-run の結果レポート** (`docs/04_test_results/<FID>/implementation-green-confirmation.md`) を Read して Section A を判定。**テスト実行は本スキルではしない** (test-run の責務)。
