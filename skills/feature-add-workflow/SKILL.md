...
3. 選定結果と理由を `decisions.md` に記録し、proposal レポートの「選定結果」を埋める
...
1. **🛑 設計差分をユーザに提示して承認を待つ**: 変更ドキュメントのパス一覧、差分表の要約、選定案、影響範囲、テスト計画の概要。応答パターンはベース §「人間チェックポイント」と同じ。approve 時は `decisions.md` 記録 + **commit** (`[feature-add-workflow] checkpoint: <FID> design approved`)
...
3. 全層 Pass + review pass で完了。**commit** (`[feature-add-workflow] <FID>: completed`)。完了報告 (変更ファイル・テスト結果サマリ・残課題)
