...
- `position: after <unit|integration|e2e>_test_review` → 当該層の cross review pass かつ open_bugs = 0 の直後に挿入 (testing のレビューは層別 3 Agent のため層を指定する)
- `position: after testing_review` → **e2e_test_review の cross pass 後 (= testing フェーズ完了直前)** と解釈する (後方互換の別名)
...
- 追加フェーズの状態も各機能の `.dev-workflow/features/<FID>/status.json` の `phases.<id>` として管理 (自動追加)
...
- インストール済みのベース Skill 群 (`~/.claude/skills/dev-workflow/` 等。本スキル `dev-workflow-overlay` 自身の定義ファイルも書き換えない) と Agent 群 (`~/.claude/agents/requirements/`, `~/.claude/agents/basic-design/`, `~/.claude/agents/implementation/`, ... の 26 個)
