...
ベース層は一切変更しない。本スキルは次の3点を上書きする:

1. **ルール上書き**: 各フェーズの作業内容に、プロジェクトルールファイル (`<project>/.dev-workflow/rules/<phase>.md`) の `ADD` / `OVERRIDE` / `DISABLE` を適用する。
2. **スキル上書き**: 同名のプロジェクトローカルスキル (`<project>/.claude/skills/<skill>/`) があれば、そちらを優先して spawn する。
3. **フェーズ追加**: `<project>/.dev-workflow/rules/extra-phases.md` で定義された追加フェーズを、フェーズ遷移に挿入する。
...
| 優先度 | パス                                                                                       |
| ------ | ------------------------------------------------------------------------------------------ |
| 1      | `<PROJECT_ROOT>/.claude/skills/<P>/SKILL.md` (プロジェクトローカル上書きスキルが存在する場合) |
| 2      | `~/.claude/skills/<P>/SKILL.md` (ユーザグローバルにインストールされたベーススキル)         |
