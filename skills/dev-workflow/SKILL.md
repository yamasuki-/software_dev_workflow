...
1. **現在の作業ディレクトリ (CWD。Cowork では選択フォルダ) を `PROJECT_ROOT` とする。** ユーザにパスの明示指定を求めない (「プロジェクトルートは <CWD> を使います」と一言共有すれば足りる)。ユーザが CWD と異なる場所を対象にしたい場合のみ、明示されたパスを使う。
...
要点 (原則: 根本原因が混入した最上流の工程からやり直し、下流へ影響範囲を連鎖更新する):
- bug-fix は上流起因の分類 (`test_design_gap` / `design_error_detailed` / `design_error_basic` / `undocumented_behavior` / `requirements_misinterpretation`) では自分で要件・設計・テスト設計を編集せず、オーケストレータに差し戻しを要請する (`target_phase` = requirements / basic_design / detailed_design / test_design)
- オーケストレータは bug-fix を一時停止 → **差し戻し先の工程を spawn しレビューゲートを通す** → **下流工程 (requirements → basic → detailed → test-design) を影響範囲に縮退した差分更新で連鎖再実施** (差分なしの段は skip 可・根拠を decisions.md に記録) → `bug.json` に結果を書き戻し → **bug-fix を Step 3 から再 spawn**
- bug-fix Step 5 と verified 後の `testing (mode=retry)` で **影響範囲 (`impacted_FIDs` × 影響層) を必ずテスト** する
