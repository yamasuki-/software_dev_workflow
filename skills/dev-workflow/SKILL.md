...
| **[Layer 1: 単体]** 全機能 `phases.testing.layers.unit.status` が `pending`                            | **`testing` (layer=unit)** 機能ごと並行 spawn                    |
...
- `basic-design`, `detailed-design`, `test-design`, `test-implementation`, `implementation`, `testing`, `bug-fix`
- `basic-design-review`, `detailed-design-review`, `test-design-review`, `test-implementation-review`, `implementation-review`, `unit-test-review`, `integration-test-review`, `e2e-test-review`, `bug-fix-review`
- `auto-check`, `test-run`
...
`Task(subagent_type="test-run", ...)` で呼び出す。手順は agent の system prompt に組み込み済み。
...
(cross の場合 docs/06_reviews/cross/<phase>-auto-check.md) に保存。
