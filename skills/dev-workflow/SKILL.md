...
flowchart TD
    ...
    subgraph Batch [各フェーズはバッチ: 全機能を同時に処理 → 横断レビュー]
      direction TB
      Orch -->|parallel spawn per FID| DD[detailed-design F001..Fn]
      Orch -->|spawn 1回| DDR[detailed-design-review<br/>全機能横断]
      Orch -->|parallel spawn per FID| TD[test-design F001..Fn]
      Orch -->|spawn 1回| TDR[test-design-review<br/>全機能横断]
      Orch -->|parallel spawn per FID| TI[test-implementation<br/>TDD Red, F001..Fn]
      Orch -->|spawn 1回| TIR[test-implementation-review<br/>全機能横断]
      Orch -->|COMMON 先行→ parallel per FID| Impl[implementation<br/>TDD Green, F001..Fn]
      Orch -->|spawn 1回| IR[implementation-review<br/>全機能横断]
      Orch -->|parallel spawn per FID| Test[testing F001..Fn]
      Orch -->|spawn 1回| TR[testing-review<br/>全機能横断]
    end
...
| 全機能の `detailed_design` 完了、`detailed_design.review` 未実行  | **`detailed-design-review`** (全機能横断) |
| 全機能のうち `test_design` 未完なものがある                       | `test-design` (機能ごとに並行 spawn) |
| 全機能の `test_design` 完了、`test_design.review` 未実行          | **`test-design-review`** (全機能横断) |
| 全機能のうち `test_implementation` 未完なものがある               | `test-implementation` (機能ごとに並行 spawn) |
| 全機能の `test_implementation` 完了、`...review` 未実行           | **`test-implementation-review`** (全機能横断) |
| 共通実装の特定タスクが pending                                    | `implementation` (擬似機能 `COMMON` を最初に処理) |
| 全機能のうち `implementation` 未完なものがある                    | `implementation` (機能ごとに並行 spawn) |
| 全機能の `implementation` 完了、`implementation.review` 未実行    | **`implementation-review`** (全機能横断) |
| 全機能のうち `testing` 未完なものがある                           | `testing` (機能ごとに並行 spawn) |
| 全機能の `testing` 完了、`testing.review` 未実行                  | **`testing-review`** (全機能横断) |
| `testing-review` の判定で `open_bugs` が非空                      | `bug-fix` (バグごと)         |
