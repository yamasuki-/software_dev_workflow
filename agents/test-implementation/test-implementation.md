...
> `dev-workflow` / `dev-workflow-overlay` skill から `Task(subagent_type="test-implementation", ...)` で spawn される。
...
本スキルはフェーズバッチで呼ばれる。複数機能のテストコード作成が同時並行で進む:

1. **他機能の同フェーズ成果物 (`tests/{unit,integration,e2e}/F<他>/...`) があれば Read** し、テストファイル命名・テスト関数命名・モック/フィクスチャ作法を揃える
2. **共通フィクスチャ/共通テストヘルパー** を発見したら `open-questions.md` に `[COMMON 候補]` で追記
3. 既存の `COMMON` テストコード (`tests/{unit,integration,e2e}/COMMON/`) があれば、自機能のテストはそれを import/再利用する
