...
- **Bash は読み取り専用の観察に限定** (例: `grep` 相当の走査 / 依存定義の確認 / 既存テストの一覧表示・実行によるカバー状況確認)。ファイルの作成・変更・削除、ビルド・インストール等の副作用を残すコマンドは実行しない
- **Write で書いてよいのはレポート (`docs/07_analysis/<解析ID>/`) のみ**
- git の状態確認 (`git status` / `git diff` / `git log`) は読み取りなので可
