...
> `dev-workflow` / `dev-workflow-overlay` skill から `Task(subagent_type="implementation", ...)` で spawn される。
...
## バッチ実行と横断認識 (共通実装の取り扱い)

本スキルはフェーズバッチで呼ばれる。複数機能の実装が同時並行で進むため、特に **コードの重複と共通化** に注意する:

1. **COMMON 機能が立っている場合は最優先で先行実装する**:
   - オーケストレータは `COMMON` の `implementation` を他機能より先に spawn する
   - 他機能の自分は `src/common/` (または `COMMON` で決められた配置) のモジュールを **import して使う**。重複実装しない
2. **他機能のコード (`src/...`) が並行作成されている可能性**:
   - 既存のソースを Read で確認し、命名規約・モジュール構成・例外処理パターンを揃える
   - 既存に類似モジュールがあれば、そちらを使えないか検討する
3. **共通化候補を発見したら `open-questions.md` に `[COMMON 候補]` で追記**:
   - 重複コードを書きそうな場合、書く前にオーケストレータに通知する
   - レビューが `COMMON` への切り出しを判断する
4. 本フェーズで新しい `COMMON` 機能を勝手に立てない (それはレビューの責務)
