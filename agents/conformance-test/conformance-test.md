...
4. git 操作をしない (状態確認の `git status` / `git diff` は読み取りなので可)
5. **Bash はテスト実行と読み取り観察に限定**。本 Agent が書いてよいのはテストコード (`tests/`) と結果レポートのみ。**終了前に `git status` / `git diff` で src/ に変更が無いことを確認** してから戻り値を返す
