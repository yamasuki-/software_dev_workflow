...
このスキルセット (`C:\Users\yamas\github\claudecode_settings`) を任意の新規プロジェクトに適用する手順。
...
Copy-Item -Recurse "C:\Users\yamas\github\claudecode_settings\skills\*" $ClaudeSkills

# 特定プロジェクトでだけ使うなら
$ProjectSkills = "C:\Users\yamas\projects\my-new-app\.claude\skills"
New-Item -ItemType Directory -Force -Path $ProjectSkills | Out-Null
Copy-Item -Recurse "C:\Users\yamas\github\claudecode_settings\skills\*" $ProjectSkills
```
...
cp -R /path/to/claudecode_settings/skills/* ~/.claude/skills/

# プロジェクト個別
mkdir -p /path/to/my-new-app/.claude/skills
cp -R /path/to/claudecode_settings/skills/* /path/to/my-new-app/.claude/skills/
```
...
```
C:\Users\yamas\github\claudecode_settings\skills\dev-workflow\SKILL.md
を読んで、その指示に従って開発ワークフローを始めてください。
プロジェクトルートは C:\Users\yamas\my-new-app です。
```
...
ユーザ:
  ...
  認証なし。データは SQLite。
  プロジェクトルート: C:\Users\yamas\projects\task-api
```
...
プロジェクトルート: C:\Users\yamas\projects\legacy-app
...
プロジェクトルート: C:\Users\yamas\projects\inventory-app

要件は私が USDM 形式で作成済みで、下記に置いてある:
  C:\Users\yamas\projects\inventory-app\docs\requirements\usdm.md
...
私が作成した USDM 形式の差分要求書を下記に置いた:
  C:\Users\yamas\projects\inventory-app\docs\requirements\usdm-rev2.md
