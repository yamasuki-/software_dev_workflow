...
| `bug-fix`                      | 不具合修正 (原因調査は `bug-investigation` の独立レポートを引き継ぎ Step 2 から → 根本原因の混入工程 + 影響範囲 `impacted_FIDs` の特定とハンドオフ (最上流 requirements まで差し戻し可・下流工程は影響範囲で連鎖更新) → 再現テスト Red + 前工程補強 (TDD) → コード修正 → テスト実施 (機能内 3 層 + 影響範囲必須) の5ステップ反復ループ) |
